  ...
  const Icons = {
    ...
    Graduation: () => (
      <svg className="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M22 10 12 5 2 10l10 5 10-5Z" />
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 12v5c3 2 9 2 12 0v-5" />
      </svg>
    ),
    Certificate: () => (
      <svg className="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 3h10a2 2 0 0 1 2 2v14l-4-2-3 2-3-2-4 2V5a2 2 0 0 1 2-2Z" />
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 8h8M8 12h6" />
      </svg>
    ),
    Sun: () => (
      <svg className="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    Phone: () => (
      <svg className="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5a2 2 0 0 1 2-2h3.28a1 1 0 0 1 .95.68l1.07 3.21a1 1 0 0 1-.24 1l-1.69 1.69a16 16 0 0 0 6.73 6.73l1.69-1.69a1 1 0 0 1 1-.24l3.21 1.07a1 1 0 0 1 .68.95V19a2 2 0 0 1-2 2h-1C9.72 21 3 14.28 3 6V5z" />
      </svg>
    ),
    Briefcase: () => (
      <svg className="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  const projects = [
    {
      title: "StrategicAI — AI-Powered Company Intelligence & Recommendation Platform",
      description:
  ...
  const certifications = [
    ...
    { name: "Data Analysis with Pandas and Python", issuer: "Packt", year: "2025" },
    { name: "GenAI-Powered Data Analytics Job Simulation", issuer: "TATA", year: "2025" },
  ...
  function SigmoidLogo({ compact = false }: { compact?: boolean }) {
    return (
      <a href="#" className="group inline-flex items-center gap-3" aria-label="Home">
        <div className="relative flex h-12 w-12 items-center justify-center">
          <div className="relative h-12 w-12 transition-all duration-500 ease-out group-hover:scale-110">
            <svg
              viewBox="0 0 120 120"
              className="h-full w-full origin-center transition-transform duration-[1400ms] ease-[cubic-bezier(0.34,1.56,0.64,1)] group-hover:rotate-[360deg] drop-shadow-[0_2px_4px_rgba(0,0,0,0.3)]"
            >
              <defs>
              ...
              {[0, 45, 90, 135, 180, 225, 270, 315].map((angle) => (
                <g key={`handle-${angle}`} transform={`rotate(${angle} 60 60)`}>
                  <path d="M 60 4 C 56 4 53 8 53 14 C 53 18 56 22 60 22 C 64 22 67 18 67 14 C 67 8 64 4 60 4 Z" fill="url(#wheel-grad)" />
                  <ellipse cx="57" cy="11" rx="1.5" ry="3" fill="white" opacity="0.25" />
                </g>
              ))}
              <circle cx="60" cy="60" r="38" fill="none" stroke="url(#wheel-grad)" strokeWidth="9" />
              <circle cx="60" cy="60" r="38" fill="none" stroke="#0a0a0a" strokeWidth="1" opacity="0.4" />
              <circle cx="60" cy="60" r="34" fill="none" stroke="#0a0a0a" strokeWidth="0.8" opacity="0.3" />
              <path d="M 32 42 A 38 38 0 0 1 88 42" fill="none" stroke="white" strokeWidth="1.2" opacity="0.25" strokeLinecap="round" />
              {[0, 45, 90, 135, 180, 225, 270, 315].map((angle) => (
                <g key={`spoke-${angle}`} transform={`rotate(${angle} 60 60)`}>
                  <rect x="58.5" y="22" width="3" height="32" fill="url(#wheel-grad)" rx="1" />
                  <rect x="58.5" y="22" width="0.8" height="32" fill="white" opacity="0.18" />
                </g>
              ))}
              <circle cx="60" cy="60" r="9" fill="#0a0a0a" />
              <circle cx="60" cy="60" r="9" fill="none" stroke="#064e3b" strokeWidth="0.5" />
              <circle cx="60" cy="60" r="4" fill="url(#hub-grad)" />
              <circle cx="60" cy="60" r="4" fill="none" stroke="#064e3b" strokeWidth="0.8" />
        ...
        {!compact && (
          <span className="flex flex-col justify-center leading-tight">
            <span className="block text-sm font-bold tracking-[0.22em] text-slate-900 transition-colors dark:text-white">SRUTIK</span>
            <span className="mt-0.5 block text-[11px] font-semibold uppercase tracking-[0.28em] text-emerald-600 dark:text-emerald-400">NANDANIYA</span>
          </span>
        )}
      </a>
    );
  ...
  function SkillChip({ skill }: { skill: TechnicalSkill }) {
    const iconUrl = fullColorIcons[skill.icon] || "https://img.icons8.com/color/48/code.png";
    return (
    ...
    return (
      <div className="relative overflow-hidden py-2 mask-image: linear-gradient(to right,transparent,black 5%,black 95%,transparent);">
        <div
          ...
          style={group.direction === "right" ? { transform: "translateX(-33.333%)" } : undefined}
        >
          {tripled.map((skill, i) => (
  ...
  function Navbar({ theme, toggleTheme }: { theme: Theme; toggleTheme: () => void }) {
    ...
    const activeSection = useActiveSection(["about", "experience", "projects", "skills", "education", "contact"]);
    ...
    return (
      <nav className={`fixed left-0 right-0 top-0 z-[60] transition-all duration-300 ${isScrolled ? "bg-white/90 dark:bg-slate-950/92 shadow-lg shadow-black/5 dark:shadow-slate-950/30 backdrop-blur-md" : "bg-transparent"}`}>
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="flex h-20 items-center justify-between">
            ...
            <div className="hidden items-center space-x-7 md:flex">
              ...
              <button type="button" onClick={toggleTheme} className="rounded-lg p-2 text-slate-500 transition-colors hover:bg-slate-100 dark:text-slate-400 dark:hover:bg-slate-800" aria-label="Toggle theme">
                {theme === "dark" ? <Icons.Sun /> : <Icons.Moon />}
              </button>

              <button type="button" onClick={downloadResume} className="inline-flex items-center gap-2 rounded-full bg-emerald-500 px-4 py-3.5 font-semibold text-white shadow-lg shadow-emerald-500/25 transition-all duration-200 hover:bg-emerald-600 hover:shadow-xl hover:shadow-emerald-500/30">
                <Icons.Download />
                Download Resume
              </button>
            </div>
            ...
            <div className="flex items-center gap-2 md:hidden">
              <button type="button" onClick={toggleTheme} className="rounded-lg p-2 text-slate-500 dark:text-slate-400" aria-label="Toggle theme">
                {theme === "dark" ? <Icons.Sun /> : <Icons.Moon />}
              </button>
              <button className="relative flex h-10 w-10 items-center justify-center rounded-lg text-slate-700 transition-colors hover:bg-slate-100 dark:text-white dark:hover:bg-slate-800" onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} aria-label="Toggle menu">
                <span className={`menu-icon-swap absolute ${isMobileMenuOpen ? "opacity-0 rotate-90 scale-50" : "opacity-100 rotate-0 scale-100"}`}><Icons.Menu /></span>
                <span className={`menu-icon-swap absolute ${isMobileMenuOpen ? "opacity-100 rotate-0 scale-100" : "opacity-0 -rotate-90 scale-50"}`}><Icons.Close /></span>
              </button>
            </div>
          </div>
        </div>
        ...
        {isMobileMenuOpen && (
          <>
            <div className="mobile-menu-enter fixed inset-0 top-20 z-40 bg-black/20 backdrop-blur-md md:hidden" onClick={() => setIsMobileMenuOpen(false)} />
...
>
        ...
        <div className="relative z-10 mx-auto w-full max-w-7xl px-5 sm:px-6 lg:px-8">
          <div className="mx-auto max-w-4xl text-center">
            ...
            <p className="hero-fade hero-fade-3 mx-auto mb-8 max-w-2xl px-2 text-base font-medium leading-7 text-slate-500 sm:text-lg dark:text-slate-400">
              Designing intelligence with data. Computer Science graduate specializing in Python, Machine Learning, and practical AI systems that turn complex data into useful products.
            </p>
            <div className="hero-fade hero-fade-4 mb-14 flex flex-col items-center justify-center gap-4 sm:flex-row">
              <a href="#projects" className="inline-flex w-full justify-center gap-2 rounded-full bg-emerald-500 px-6 py-3 sm:w-auto font-semibold text-white shadow-lg shadow-emerald-500/25 transition-all duration-200 hover:bg-emerald-600 hover:shadow-xl hover:shadow-emerald-500/30">
  ...
  function AboutSection() {
    return (
      <section id="about" className="relative bg-white/80 py-16 sm:py-20 dark:bg-slate-950/80">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="reveal mb-16 text-center">
            <h2 className="mb-4 text-3xl font-bold text-slate-900 sm:text-4xl dark:text-white">About <span className="text-emerald-500">Me</span></h2>
  ...
  >
                ...
                <div className="absolute inset-6 overflow-hidden rounded-3xl bg-slate-100 shadow-2xl transition-transform group-hover:scale-[1.02] dark:bg-slate-800">
                  <img
                    ...
                    onError={(e) => {
                      ...
                      if (el.parentElement) {
                        el.parentElement.innerHTML = '<div class="flex flex-col items-center justify-center h-full text-center"><div class="text-6xl mb-2">👨‍💻</div><p class="text-xs text-slate-400">Profile Image</p></div>';
                      }
                    }}
                  />
                </div>
            ...
            <div className="space-y-6">
              <h3 className="text-2xl font-bold text-slate-900 dark:text-white">Data Science & AI/ML Enthusiast</h3>
              <p className="text-lg leading-relaxed text-slate-600 dark:text-slate-400">
                I am a Computer Science graduate with a strong focus on Data Science, Machine Learning, and Artificial Intelligence. My work is driven by curiosity, clear problem-solving, and a commitment to building practical data-driven solutions.
              </p>
              <p className="leading-relaxed text-slate-600 dark:text-slate-400">
                With hands-on experience in Python, machine learning algorithms, exploratory analysis, and AI tools, I have developed projects across prediction systems, generative AI, gesture recognition, healthcare assistance, and management automation.
              </p>
            </div>
          </div>
        </div>
  ...
  function ExperienceSection() {
    return (
      <section id="experience" className="relative bg-slate-50/80 py-16 sm:py-20 dark:bg-slate-900/80">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="reveal mb-16 text-center">
            <h2 className="mb-4 text-3xl font-bold text-slate-900 sm:text-4xl dark:text-white">Internship & <span className="text-emerald-500">Training</span></h2>
            <div className="mx-auto h-1.5 w-20 rounded-full bg-emerald-500" />
          </div>
            ...
            {experience.map((ex, i) => (
              ...
              >
                <div className="mb-6 flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
                  <div>
                    <h3 className="text-lg sm:text-xl font-bold text-slate-900 dark:text-white">{ex.title}</h3>
                    <p className="font-semibold text-emerald-600 dark:text-emerald-400">{ex.company}</p>
                  </div>
                ...
                <ul className="mb-6 space-y-3">
                  {ex.desc.map((d, j) => (
                    <li key={j} className="flex gap-3 text-slate-600 dark:text-slate-400">
                ...
                {ex.tech && (
                  <div className="flex flex-wrap gap-2 border-t border-slate-100 pt-4 dark:border-slate-800">
                    {ex.tech.map((t) => (
                      <span key={t} className="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-600 dark:bg-slate-800 dark:text-slate-400">
                        {t}
                      </span>
                    ))}
                  </div>
    ...
    return (
      ...
      >
        ...
        <div className="relative h-44 overflow-hidden sm:h-52">
          <div className={`absolute inset-0 bg-gradient-to-br ${project.color} opacity-90 transition-transform duration-500 group-hover:scale-105`} />

          {/* Abstract decorative shapes */}
          <svg className="absolute inset-0 h-full w-full" viewBox="0 0 400 208" fill="none">
            {/* Large circle */}
            ...
            {/* Geometric rectangle */}
            <rect x="160" y="60" width="50" height="50" rx="12" fill="white" opacity="0.07" transform="rotate(15 185 85)" />
            <rect x="260" y="120" width="35" height="35" rx="8" fill="white" opacity="0.06" transform="rotate(-10 277 137)" />
            {/* Connecting lines */}
            <line x1="80" y1="160" x2="185" y2="85" stroke="white" strokeWidth="1" opacity="0.1" />
            <line x1="185" y1="85" x2="340" y2="40" stroke="white" strokeWidth="1" opacity="0.08" />
            <line x1="185" y1="85" x2="277" y2="137" stroke="white" strokeWidth="1" opacity="0.08" />
          ...
          <div className="absolute left-5 top-5 flex flex-wrap gap-1.5">
            {project.tags.slice(0, 2).map((t) => (
              <span key={t} className="rounded-full bg-white/20 px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-widest text-white backdrop-blur-sm">
                {t}
              </span>
            ))}
          </div>
        ...
        <div className="flex flex-1 flex-col p-7">
          ...
          <div className="mb-5 flex flex-wrap gap-1.5">
            {project.tags.map((t) => (
              <span key={t} className="rounded-full bg-emerald-50 px-3 py-1 text-[10px] font-bold uppercase tracking-widest text-emerald-600 dark:bg-emerald-500/10 dark:text-emerald-400">
                {t}
              </span>
            ))}
          </div>
  ...
  function ProjectsSection() {
    return (
      <section id="projects" className="relative bg-white/80 py-16 sm:py-20 dark:bg-slate-950/80">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="reveal mb-16 text-center">
            <h2 className="mb-4 text-3xl font-bold text-slate-900 sm:text-4xl dark:text-white">Selected <span className="text-emerald-500">Works</span></h2>
  ...
  function SkillsSection() {
    return (
      <section id="skills" className="overflow-hidden bg-slate-50/80 py-16 sm:py-20 dark:bg-slate-900/80">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="reveal mb-12 text-center">
            <h2 className="mb-4 text-3xl font-bold text-slate-900 sm:text-4xl dark:text-white">Technical <span className="text-emerald-500">Expertise</span></h2>
            <div className="mx-auto mt-4 h-1.5 w-20 rounded-full bg-emerald-500" />
          </div>
        </div>
          ...
          {technicalSkillGroups.map((g, i) => (
            <div key={g.title} className="reveal" data-delay={Math.min(i + 1, 5)}>
              <div className="mx-auto mb-4 max-w-7xl px-4 text-center sm:px-6 lg:px-8">
                <h3 className="text-lg font-bold uppercase tracking-widest text-slate-800 dark:text-slate-100">{g.title}</h3>
              </div>
  ...
  function EducationSection() {
    return (
      <section id="education" className="relative bg-white/80 py-16 sm:py-20 dark:bg-slate-950/80">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="reveal mb-16 text-center">
            <h2 className="mb-4 text-3xl font-bold text-slate-900 sm:text-4xl dark:text-white">Education & <span className="text-emerald-500">Certifications</span></h2>
            <div className="mx-auto h-1.5 w-20 rounded-full bg-emerald-500" />
          </div>
          <div className="grid gap-12 lg:grid-cols-2">
            <div>
              <h3 className="mb-8 flex items-center gap-3 text-lg sm:text-xl font-bold uppercase tracking-wider text-slate-900 dark:text-white">
                <span className="flex h-12 w-12 items-center justify-center rounded-2xl bg-emerald-500 text-white shadow-lg shadow-emerald-500/20"><Icons.Graduation /></span>
                Education
              </h3>
              <div className="space-y-8">
                {education.map((e) => (
                  <div key={e.degree} className="relative border-l-2 border-emerald-500/30 pl-10 transition-colors hover:border-emerald-500">
                    ...
                    <div className="rounded-3xl border border-slate-200 bg-slate-50 p-5 sm:p-8 dark:border-slate-800 dark:bg-slate-900/50">
                      <span className="text-sm font-bold uppercase tracking-widest text-emerald-600 dark:text-emerald-400">{e.period}</span>
                      <h4 className="mb-2 mt-1 text-lg sm:text-xl font-bold text-slate-900 dark:text-white">{e.degree}</h4>
                      <p className="text-lg font-medium text-slate-600 dark:text-slate-400">{e.institution}</p>
                      <p className="mt-1 text-sm text-slate-400 dark:text-slate-500">{e.location}</p>
                      <div className="mt-4 inline-block rounded-xl bg-slate-200 px-3 py-1.5 text-xs sm:px-4 sm:py-2 sm:text-sm font-bold text-slate-700 dark:bg-slate-800 dark:text-emerald-400">{e.details}</div>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div>
              <h3 className="mb-8 flex items-center gap-3 text-lg sm:text-xl font-bold uppercase tracking-wider text-slate-900 dark:text-white">
                <span className="flex h-12 w-12 items-center justify-center rounded-2xl bg-slate-900 text-white shadow-lg dark:bg-white dark:text-slate-900"><Icons.Certificate /></span>
                Certifications
              </h3>
              <div className="space-y-4">
                {certifications.map((c) => (
                  <div key={c.name} className="rounded-3xl border border-slate-200 bg-slate-50 p-6 transition-all hover:border-emerald-500/40 dark:border-slate-800 dark:bg-slate-900/50">
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1">
                        <h4 className="mb-1.5 text-lg font-bold text-slate-900 dark:text-white">{c.name}</h4>
                        <p className="text-sm font-medium text-slate-500 dark:text-slate-400">{c.issuer}</p>
                      </div>
                      <span className="whitespace-nowrap text-xs font-bold uppercase tracking-widest text-emerald-600 dark:text-emerald-400">{c.year}</span>
                    </div>
                  </div>
                ))}
              </div>
              <div className="mt-8 text-center">
                <a href="https://drive.google.com/drive/u/4/folders/1SkYZVxpTE1JfguqfP6lyDN4Ah2yoo1lE" className="inline-flex items-center gap-3 rounded-full bg-slate-900 px-8 py-4 text-sm font-bold text-white transition-all hover:scale-105 dark:bg-white dark:text-slate-900">
  ...
  function ContactSection() {
    ...
    return (
      <section id="contact" className="relative overflow-hidden bg-white/80 py-24 dark:bg-slate-900/80">
        <div className="relative z-10 mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="reveal mb-20 text-center">
            <h2 className="mb-5 text-4xl font-bold tracking-tight text-slate-900 sm:text-5xl dark:text-white">Let's <span className="text-emerald-500">Connect</span></h2>
            ...
            <div className="grid gap-12 lg:grid-cols-5">
              <div className="space-y-8 lg:col-span-2">
                <div className="space-y-8 rounded-[2.5rem] border border-slate-200 bg-slate-50 p-10 dark:border-slate-800 dark:bg-slate-950">
                  <h3 className="text-2xl font-bold text-slate-900 dark:text-white">Contact Info</h3>
                  <div className="space-y-8">
                    <div className="flex items-start gap-5">
                      <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-emerald-500 text-white shadow-lg"><Icons.Email /></div>
                      <div>
                        <p className="mb-1 text-sm font-bold uppercase tracking-widest text-slate-400">Email</p>
                        <a href="mailto:srutikndn@gmail.com" className="text-lg font-bold text-slate-800 transition-colors hover:text-green-500 dark:text-white">srutikndn@gmail.com</a>
                      </div>
                    </div>
                    <div className="flex items-start gap-5">
                      <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-slate-900 text-white shadow-lg dark:bg-white dark:text-slate-900"><Icons.GitHub /></div>
                      <div>
                        <p className="mb-1 text-sm font-bold uppercase tracking-widest text-slate-400">GitHub</p>
                        <a href="https://github.com/SrutikNandaniya" target="_blank" rel="noopener noreferrer" className="text-lg font-bold text-slate-800 transition-colors hover:text-purple-500 dark:text-white">Srutik Nandaniya</a>
                      </div>
                    </div>
                    <div className="flex items-start gap-5">
                      <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-blue-600 text-white shadow-lg"><Icons.LinkedIn /></div>
                      <div>
                        <p className="mb-1 text-sm font-bold uppercase tracking-widest text-slate-400">LinkedIn</p>
                        <a href="https://www.linkedin.com/in/srutiknandaniya/" target="_blank" rel="noopener noreferrer" className="text-lg font-bold text-slate-800 transition-colors hover:text-blue-500 dark:text-white">Srutik Nandaniya</a>
                      </div>
                    </div>
                  </div>
                </div>
...
>
    ...
    return (
      <footer className="border-t border-slate-200 bg-slate-50 py-12 dark:border-slate-800 dark:bg-slate-950">
        <div className="mx-auto max-w-7xl px-4 text-center sm:px-6 lg:px-8">
          <p className="text-sm font-bold uppercase tracking-widest text-slate-400 dark:text-slate-500">© {new Date().getFullYear()} Srutik Nandaniya</p>
        </div>
      </footer>
    );
  }
  ...
  function ScrollProgress() {
    ...
    return (
      <div className="fixed left-0 right-0 top-0 z-[60] h-[3px] bg-slate-200 dark:bg-slate-800">
        <div className="h-full bg-gradient-to-r from-emerald-400 to-emerald-600 transition-[width] duration-75 ease-out dark:from-emerald-300 dark:to-emerald-500" style={{ width: `${progress}%` }} />
      </div>
    );
  }
  ...
  export default function App() {
    ...
    useRevealOnScroll();
    return (
      <div className={`relative min-h-screen ${theme === "dark" ? "dark bg-slate-950 text-slate-100" : "bg-white text-slate-900"} selection:bg-emerald-500/30 selection:text-emerald-900 font-sans`}>
